...
/**
 * 
 * A singleton flyweight factory responsible for chopping the data up and
 * sending it to the corresponding chunks(which are flyweights ala the flyweight
 * pattern) for processing. This will sequentially read a 3ds file, load or skip
 * chunks and subchunks and initialize the data for the chunks.
 * <p>
 * Retrieved data may be stored as state in the ChunkChopper via
 * {@link #pushData} for use by other chunks.
 * <p>
 * Features not supported; unknown chunks are skipped.
 * 
 * 
 * Max 3ds loader for jME3, modified from ChunkChopper
 * 
 * @author yanmaoyuan
 */
public class Max3dsLoader implements AssetLoader {
	private AssetManager manager = null;
	private AssetKey key = null;

	private Logger logger = Logger.getLogger(Max3dsLoader.class.getName());
	 ...
	 * constructs the 3D entities represented within. A Chunk is a little endian
	 * data structure consists of a 6 byte header followed by subchunks and or
	 * data. The first short int(little endian) represent the id of the chunk.
	 * The next int represent the total length of the chunk(total of data,
	 * subchunks and chunk header).
	 * <p>
	 * The first chunk is the main chunk (id=4D4D) and its length is always the
	 * length of the file. It only contains sub chunks. Other chunks may contain
	 * data, subchunks or both. If the format of a chunk is unknown skipped.
	 * <p>
	 * Subclasses of chunk will all automagically load the subchunks. It is the
	 * programmers responsibility to ensure that the data preceeding the
	 * subchunks is loaded or skipped as required and that something useful is
	 * done with the data. If data from the subchunks is needed in order to
	 * initialize components then that code should be placed in
	 * {@link Chunk#initialize}. Otherwise the data may be dealt with in
	 * {@link Chunk#loadData}. Also, if a chunk has data preceeding its
	 * subchunks it communicates how many bytes long that data is by returning
	 * it from loadData.
	 * <p>
	 * This chopper reads a file in order from beginning to end
	 * 
	 * @param inputStream
	 *            the stream with the data to be parsed.
	 * @param loader
	 *            the loader that will be configured from the data.
	 * @param modelName
	 *            name of the model file for display purposes.
	 * @param modelSize
	 ...
	 * 
	 * @param channel
	 *            the file channel to load the data from
	 * @return a direct byte buffer containing all the data of the channel at
	 *         position 0
	 */
	 ...
	 * 
	 * @param light
	 *            the light to add to the scene base.
	 */
	 ...
	 * up; this returns new Vector3f(x,-z,y)
	 * 
	 */
