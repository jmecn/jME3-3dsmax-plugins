...
public class KeyframeControl extends AbstractControl {
    ...
    /**
     * Called by controlUpdate, and itself recursivly. Will, when completed, change
    ...
    private void fillScales() {
        for (int objIndex = 0; objIndex < numObjects; objIndex++) {
            ...
            for (start = 0; start < keyframes.size(); start++) {
                if (keyframes.get(start).usedScale.get(objIndex)) break;
            }
            ...
            keyframes.get(lastgood).look[objIndex].getScale(unSyncbeginPos);

            for (int i = lastgood + 1; i < keyframes.size(); i++) {
                keyframes.get(i).look[objIndex].setScale(unSyncbeginPos);
            }
        }
    }
